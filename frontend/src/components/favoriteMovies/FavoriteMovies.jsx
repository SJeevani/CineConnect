...
function FavoriteMovies() {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            try {
                const response = await axiosWithToken.get(`http://localhost:4000/user-api/movies/favorites/${currentUser.username}`);
    ...
    return (
        <div className="movies-container">
            <h1 className='mt-5 text-center fw-bold'>Favorite Movies</h1>
            {movies.length > 0 ? (
                <>
